...
module.exports = {
    ...
    run: async(client, message, args) => {
       /* const mapping = {
        ...
        var etiket =  message.guild.members.cache.filter(s => !s.bot).filter(member => member.user.discriminator == "0041").size;
        var toplamAile = message.guild.members.cache.filter(s => !s.bot).filter(member => member.user.username.includes(tag) || member.user.discriminator == "0041").size;
       ...
       /* var emotoplamüye = `${toplamüye}`.split("").map(c => mapping[c] || c).join("")
        ...
        const embed = new MessageEmbed()
            ...
            .setFooter('Łâventâ Aile Sunucusu')
